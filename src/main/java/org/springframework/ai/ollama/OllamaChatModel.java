	...
	static ChatResponseMetadata from(OllamaApi.ChatResponse response, ChatResponse previousChatResponse) {
		...
		if (previousChatResponse != null && previousChatResponse.getMetadata() != null) {
			if (previousChatResponse.getMetadata().get(METADATA_EVAL_DURATION) != null) {
				evalDuration = (evalDuration != null) ? evalDuration.plus(previousChatResponse.getMetadata().get(METADATA_EVAL_DURATION)) : previousChatResponse.getMetadata().get(METADATA_EVAL_DURATION);
			}
			if (previousChatResponse.getMetadata().get(METADATA_PROMPT_EVAL_DURATION) != null) {
				promptEvalDuration = (promptEvalDuration != null) ? promptEvalDuration
					.plus(previousChatResponse.getMetadata().get(METADATA_PROMPT_EVAL_DURATION)) : previousChatResponse.getMetadata().get(METADATA_PROMPT_EVAL_DURATION);
			}
			if (previousChatResponse.getMetadata().get(METADATA_LOAD_DURATION) != null) {
				loadDuration = (loadDuration != null) ? loadDuration.plus(previousChatResponse.getMetadata().get(METADATA_LOAD_DURATION)) : previousChatResponse.getMetadata().get(METADATA_LOAD_DURATION);
			}
			if (previousChatResponse.getMetadata().get(METADATA_TOTAL_DURATION) != null) {
				totalDuration = (totalDuration != null) ? totalDuration.plus(previousChatResponse.getMetadata().get(METADATA_TOTAL_DURATION)) : previousChatResponse.getMetadata().get(METADATA_TOTAL_DURATION);
			}
