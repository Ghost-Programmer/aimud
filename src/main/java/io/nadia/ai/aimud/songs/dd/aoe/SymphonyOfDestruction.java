...
public class SymphonyOfDestruction extends Song {
     ...
     */
    public SymphonyOfDestruction(SkillService skillService, MobileService mobileService, CommunicationService communicationService, EffectService effectService) {
    ...
    public boolean sing(Mobile mobile, Song song, Mobile primaryTarget) {
        ...
        if (mobile.getUserId() != null) this.communicationService.sendTextMessage(mobile, String.format("\n\nYou tear the air apart around %s with Symphony of Destruction!", primaryTarget.getName()));
        this.communicationService.roomMessage(mobile, String.format("\n\n%s shakes the entire room violently with Symphony of Destruction!", mobile.getName()));
        ...
        for (Mobile tgt : targets) {
            ...
            if (tgt.getUserId() != null) {
                this.communicationService.sendTextMessage(tgt, String.format("\n\nYou are struck by %s's explosive Symphony of Destruction for %d sonic damage!", mobile.getName(), damage));
            }
