...
/**
 * Implementation of the discordantstrike bard song.
 */
@Component
@BardSong(name = "discordantstrike")
public class DiscordantStrike extends Song {

    /**
     * Constructs the discordantstrike dependencies.
     *
     ...
     */
    public DiscordantStrike(SkillService skillService, MobileService mobileService, CommunicationService communicationService, EffectService effectService) {
     ...
     */
    @Override
    public String getSongName() { return "Discordant Strike"; }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long getSongId() { return 3501L; }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer getSongLevel() { return 25; }
    ...
    public String getDescription() {
        return "A level 25 direct damage song. Usage: sing 'discordantstrike' <target>";
    }
    ...
    public boolean sing(Mobile mobile, Song song, Mobile target) {
        ...
        if (mobile.getUserId() != null) {
            this.communicationService.sendTextMessage(mobile, String.format("\n\nYou strike %s with the reverberations of Discordant Strike for %d sonic damage!", target.getName(), damage));
        }
        if (target.getUserId() != null) {
            this.communicationService.sendTextMessage(target, String.format("\n\n%s's Discordant Strike tears through you for %d sonic damage!", mobile.getName(), damage));
        }
        this.communicationService.roomMessage(mobile, String.format("\n\n%s shreds %s with Discordant Strike!", mobile.getName(), target.getName()));
        ...
        if (target.getCurrentHp() <= 0) {
            ...
            if (mobile.getTarget() == target) this.mobileService.setTarget(mobile, null);
            this.mobileService.findAllByRoomId(target.getCurrentRoomId()).forEach(m -> m.removeHate(target.getId()));
        } else {
            if (target.getTarget() == null) if (!this.mobileService.setTarget(target, mobile)) return false;
        }

        if (target.getUserId() != null) this.mobileService.save(target).subscribe();
