...
/**
 * Implementation of the powerchord bard song.
 */
@Component
@BardSong(name = "powerchord")
public class PowerChord extends Song {

    /**
     * Constructs the powerchord dependencies.
     *
     ...
     */
    public PowerChord(SkillService skillService, MobileService mobileService, CommunicationService communicationService, EffectService effectService) {
     ...
     */
    @Override
    public String getSongName() { return "Power Chord"; }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long getSongId() { return 3502L; }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer getSongLevel() { return 50; }
    ...
    public String getDescription() {
        return "A level 50 direct damage song. Usage: sing 'powerchord' <target>";
    }
    ...
    public boolean sing(Mobile mobile, Song song, Mobile target) {
        ...
        if (mobile.getUserId() != null) {
            this.communicationService.sendTextMessage(mobile, String.format("\n\nYou strike %s with the reverberations of Power Chord for %d sonic damage!", target.getName(), damage));
        }
        if (target.getUserId() != null) {
            this.communicationService.sendTextMessage(target, String.format("\n\n%s's Power Chord tears through you for %d sonic damage!", mobile.getName(), damage));
        }
        this.communicationService.roomMessage(mobile, String.format("\n\n%s shreds %s with Power Chord!", mobile.getName(), target.getName()));
