    ...
    public boolean sing(Mobile mobile, Song song, Mobile target) {
        ...
        if (wbEffect == null) {
            if (mobile.getUserId() != null) {
                communicationService.sendTextMessage(mobile, "\n\nYour song loses its rhythm in the air.");
            }
            return false;
        }

        if (this.applyEffect(target, this.getSongSkillName(), wbEffect, tickCount)) {

            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, String.format("\n\nYou sing a fluid melody, granting water breathing to %s.",
                        mobile == target ? "yourself" : target.getName()));
            }

            if (target.getUserId() != null && mobile != target) {
                this.communicationService.sendTextMessage(target, String.format("\n\n%s sings a fluid song, granting you the ability to breathe underwater!", mobile.getName()));
            }

            this.communicationService.roomMessage(mobile, String.format("\n\nFaint aquatic gills temporarily form on %s's neck to the tune of a fluid melody.",
