...
public class ChorusOfThorns extends Song {
     ...
     */
    public ChorusOfThorns(SkillService skillService, MobileService mobileService, CommunicationService communicationService, EffectService effectService) {
    ...
    public boolean sing(Mobile mobile, Song song, Mobile primaryTarget) {
        ...
        if (effect != null) {
            if (mobile.getUserId() != null) this.communicationService.sendTextMessage(mobile, "\n\nYou aggressively sweep Chorus of Thorns around " + primaryTarget.getName() + "!");
            this.communicationService.roomMessage(mobile, "\n\n" + mobile.getName() + " tortures the room performing an agonizing rendition of Chorus of Thorns!");
