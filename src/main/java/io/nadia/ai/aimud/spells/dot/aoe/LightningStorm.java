...
/**
 * Implementation of the lightningstorm spell.
 */
@MagicSpell(name = "lightningstorm")
public class LightningStorm extends Spell {

    /**
     * Constructs the lightningstorm dependencies.
     *
     ...
     */
    public LightningStorm(SkillService skillService, MobileService mopbileService, CharacterService characterService, CommunicationService communicationService, EffectService effectService) {
    ...
    public String getDescription() {
        return "An AoE level 90 spell that unleashes Lightning Storm on targets.. Damage and duration scale with skill rank. Usage: cast 'lightningstorm' <target>";
    }
    ...
    public boolean cast(Mobile mobile, Spell spell, Mobile primaryTarget) {
        ...
        if (effect != null) {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYou unleash a massive blast of Lightning Storm starting at " + primaryTarget.getName() + "!");
            }
            this.communicationService.roomMessage(mobile, "\n\n" + mobile.getName() + " unleashes a massive blast of Lightning Storm at " + primaryTarget.getName() + "!");
            ...
            for (Mobile tgt : targets) {
                ...
                if (tgt.getUserId() != null) {
                    this.communicationService.sendTextMessage(tgt, "\n\n" + mobile.getName() + " engulfs you in Lightning Storm!");
                }
            }
            return applied;
        } else {
