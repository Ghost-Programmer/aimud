...
/**
 * Implementation of the minor fire spell.
 */
@MagicSpell(name = "minor fire")
public class MinorFire extends Spell {

    /**
     * Constructs the minor fire dependencies.
     *
     ...
     */
    public MinorFire(SkillService skillService, MobileService mopbileService, CharacterService characterService, CommunicationService communicationService, EffectService effectService) {
    ...
    public String getDescription() {
        return "A level 4 spell that ignites the target in flames. Damage and duration scale with skill rank. Usage: cast 'minor fire' <target>";
    }
    ...
    public boolean cast(Mobile mobile, Spell spell, Mobile target) {
        ...
        if (effect != null) {
            ...
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYou cast Minor Fire on " + target.getName() + ".");
            }
            if (target.getUserId() != null) {
                this.communicationService.sendTextMessage(target, "\n\n" + mobile.getName() + " casts Minor Fire on you!");
            }
            this.communicationService.roomMessage(mobile, "\n\n" + mobile.getName() + " casts Minor Fire on " + target.getName() + ".");
            return true;
        } else {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYour spell fizzles.");
