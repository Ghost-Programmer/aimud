...
/**
 * Implementation of the major cold spell.
 */
@MagicSpell(name = "major cold")
public class MajorCold extends Spell {

    /**
     * Constructs the major cold dependencies.
     *
     ...
     */
    public MajorCold(SkillService skillService, MobileService mobileService, CommunicationService communicationService, EffectService effectService) {
    ...
    public String getDescription() {
        return "A level 45 spell that freezes the target. Damage and duration scale with skill rank. Usage: cast 'major cold' <target>";
    }
    ...
    public boolean cast(Mobile mobile, Spell spell, Mobile target) {
        ...
        if (effect != null) {
            ...
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYou cast Major Cold on " + target.getName() + ".");
            }
            if (target.getUserId() != null) {
                this.communicationService.sendTextMessage(target, "\n\n" + mobile.getName() + " casts Major Cold on you!");
            }
            this.communicationService.roomMessage(mobile, "\n\n" + mobile.getName() + " casts Major Cold on " + target.getName() + ".");
            return true;
        } else {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYour spell fizzles.");
