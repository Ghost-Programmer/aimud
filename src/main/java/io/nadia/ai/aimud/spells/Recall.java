...
public class Recall extends Spell {
    ...
    public Recall(SkillService skillService, MobileService mobileService, CommunicationService communicationService,
    ...
    public Mobile getTarget(Mobile mobile, String[] parts) {
        if (parts.length >= 3) {
            ...
            // Search all available characters in the game for the target, not just the
            // current room
            return mobileService.getAvailableMobiles().stream()
    ...
    private boolean isSameGroup(Mobile caster, Mobile target) {
        if (caster.getId().equals(target.getId()))
            return true;
        ...
        if (casterLeader != null && casterLeader.equals(targetLeader))
            return true;
        // Or if the caster is the leader and the target follows them
        if (casterLeader == null && targetLeader != null && targetLeader.equals(caster.getId()))
            return true;
        // Or if the target is the leader and the caster follows them
        if (targetLeader == null && casterLeader != null && casterLeader.equals(target.getId()))
            return true;
