...
/**
 * Implementation of the glacialcascade spell.
 */
@MagicSpell(name = "glacialcascade")
public class GlacialCascade extends Spell {

    /**
     * Constructs the glacialcascade dependencies.
     *
     ...
     */
    public GlacialCascade(SkillService skillService, MobileService mopbileService, CharacterService characterService, CommunicationService communicationService, EffectService effectService) {
    ...
    public String getDescription() {
        return "An AoE level 55 spell that unleashes Glacial Cascade on targets.. Damage and duration scale with skill rank. Usage: cast 'glacialcascade' <target>";
    }
    ...
    public boolean cast(Mobile mobile, Spell spell, Mobile primaryTarget) {
        ...
        if (effect != null) {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYou unleash a massive blast of Glacial Cascade starting at " + primaryTarget.getName() + "!");
            }
            this.communicationService.roomMessage(mobile, "\n\n" + mobile.getName() + " unleashes a massive blast of Glacial Cascade at " + primaryTarget.getName() + "!");
            ...
            for (Mobile tgt : targets) {
                ...
                if (tgt.getUserId() != null) {
                    this.communicationService.sendTextMessage(tgt, "\n\n" + mobile.getName() + " engulfs you in Glacial Cascade!");
                }
            }
            return applied;
        } else {
