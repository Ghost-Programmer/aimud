...
@MudCommand(name = "pray 'greater lowerresistance'")
public class GreaterLowerResistance extends Prayer {

    /**
     * Constructs the pray 'greater lowerresistance' dependencies.
     *
     ...
     */
    public GreaterLowerResistance(SkillService skillService, MobileService mobileService, CharacterService characterService, CommunicationService communicationService, EffectService effectService) {
    ...
    public String getPrayerName() {
        return "Greater LowerResistance";
    }
    ...
    public String getDescription() {
        return "Lowers Magic Resist and Physical Resist. Magnitude and duration scale with your skill. Usage: pray 'greater lowerresistance' <target>";
    }
    ...
    public boolean pray(Mobile mobile, Prayer prayer, Mobile target) {
        ...
        if (applied) {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYou cast Greater LowerResistance on " + target.getName() + ", dropping their stats by " + drop + ".");
            }
            if (target.getUserId() != null) {
                this.communicationService.sendTextMessage(target, "\n\n" + mobile.getName() + " casts Greater LowerResistance on you! You feel weaker!");
            }
            this.communicationService.roomMessage(mobile, "\n\n" + mobile.getName() + " casts Greater LowerResistance on " + target.getName() + ".");
            return true;
        } else {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYour prayer fails.");
