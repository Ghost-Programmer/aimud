...
@MudCommand(name = "pray 'minor lowerresistance'")
public class MinorLowerResistance extends Prayer {

    /**
     * Constructs the pray 'minor lowerresistance' dependencies.
     *
     ...
     */
    public MinorLowerResistance(SkillService skillService, MobileService mobileService, CharacterService characterService, CommunicationService communicationService, EffectService effectService) {
    ...
    public String getDescription() {
        return "Lowers Magic Resist and Physical Resist. Magnitude and duration scale with your skill. Usage: pray 'minor lowerresistance' <target>";
    }
    ...
    public boolean pray(Mobile mobile, Prayer prayer, Mobile target) {
        ...
        if (applied) {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYou cast Minor LowerResistance on " + target.getName() + ", dropping their stats by " + drop + ".");
            }
            if (target.getUserId() != null) {
                this.communicationService.sendTextMessage(target, "\n\n" + mobile.getName() + " casts Minor LowerResistance on you! You feel weaker!");
            }
            this.communicationService.roomMessage(mobile, "\n\n" + mobile.getName() + " casts Minor LowerResistance on " + target.getName() + ".");
            return true;
        } else {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYour prayer fails.");
