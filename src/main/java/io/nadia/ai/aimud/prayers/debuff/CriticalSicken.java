...
@MudCommand(name = "pray 'critical sicken'")
public class CriticalSicken extends Prayer {

    /**
     * Constructs the pray 'critical sicken' dependencies.
     *
     ...
     */
    public CriticalSicken(SkillService skillService, MobileService mobileService, CharacterService characterService, CommunicationService communicationService, EffectService effectService) {
    ...
    public String getDescription() {
        return "Lowers Constitution. Magnitude and duration scale with your skill. Usage: pray 'critical sicken' <target>";
    }
    ...
    public boolean pray(Mobile mobile, Prayer prayer, Mobile target) {
        ...
        if (applied) {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYou cast Critical Sicken on " + target.getName() + ", dropping their stats by " + drop + ".");
            }
            if (target.getUserId() != null) {
                this.communicationService.sendTextMessage(target, "\n\n" + mobile.getName() + " casts Critical Sicken on you! You feel weaker!");
            }
            this.communicationService.roomMessage(mobile, "\n\n" + mobile.getName() + " casts Critical Sicken on " + target.getName() + ".");
            return true;
        } else {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYour prayer fails.");
