...
/**
 * Implementation of the smite prayer.
 */
@Component
@DivinePrayer(name = "smite")
public class Smite extends Prayer {

    /**
     * Constructs the smite dependencies.
     *
     ...
     */
    public Smite(SkillService skillService, MobileService mobileService, CommunicationService communicationService, EffectService effectService) {
     ...
     */
    @Override
    public String getPrayerName() { return "Smite"; }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long getPrayerId() { return 2501L; }

    /**
     * {@inheritDoc}
     */
    @Override
    public Integer getPrayerLevel() { return 25; }
    ...
    public String getDescription() {
        return "A level 25 direct damage prayer. Usage: pray 'smite' <target>";
    }
    ...
    public boolean pray(Mobile mobile, Prayer prayer, Mobile target) {
        ...
        if (mobile.getUserId() != null) {
            this.communicationService.sendTextMessage(mobile, String.format("\n\nYou call down Smite on %s for %d holy damage!", target.getName(), damage));
        }
        if (target.getUserId() != null) {
            this.communicationService.sendTextMessage(target, String.format("\n\n%s calls down Smite on you for %d holy damage!", mobile.getName(), damage));
        }
        this.communicationService.roomMessage(mobile, String.format("\n\n%s strikes %s with Smite!", mobile.getName(), target.getName()));
