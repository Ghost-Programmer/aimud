...
@MudCommand(name = "pray 'greater weaken'")
public class GreaterWeaken extends Prayer {

    /**
     * Constructs the pray 'greater weaken' dependencies.
     *
     ...
     */
    public GreaterWeaken(SkillService skillService, MobileService mobileService, CharacterService characterService, CommunicationService communicationService, EffectService effectService) {
    ...
    public String getDescription() {
        return "Lowers Strength and Dexterity. Magnitude and duration scale with your skill. Usage: pray 'greater weaken' <target>";
    }
    ...
    public boolean pray(Mobile mobile, Prayer prayer, Mobile target) {
        ...
        if (applied) {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYou cast Greater Weaken on " + target.getName() + ", dropping their stats by " + drop + ".");
            }
            if (target.getUserId() != null) {
                this.communicationService.sendTextMessage(target, "\n\n" + mobile.getName() + " casts Greater Weaken on you! You feel weaker!");
            }
            this.communicationService.roomMessage(mobile, "\n\n" + mobile.getName() + " casts Greater Weaken on " + target.getName() + ".");
            return true;
        } else {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYour prayer fails.");
