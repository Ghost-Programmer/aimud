...
@MudCommand(name = "pray 'critical paperarmor'")
public class CriticalPaperArmor extends Prayer {

    /**
     * Constructs the pray 'critical paperarmor' dependencies.
     *
     ...
     */
    public CriticalPaperArmor(SkillService skillService, MobileService mobileService, CharacterService characterService, CommunicationService communicationService, EffectService effectService) {
    ...
    public String getDescription() {
        return "Lowers Armor. Magnitude and duration scale with your skill. Usage: pray 'critical paperarmor' <target>";
    }
    ...
    public boolean pray(Mobile mobile, Prayer prayer, Mobile target) {
        ...
        if (applied) {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYou cast Critical PaperArmor on " + target.getName() + ", dropping their stats by " + drop + ".");
            }
            if (target.getUserId() != null) {
                this.communicationService.sendTextMessage(target, "\n\n" + mobile.getName() + " casts Critical PaperArmor on you! You feel weaker!");
            }
            this.communicationService.roomMessage(mobile, "\n\n" + mobile.getName() + " casts Critical PaperArmor on " + target.getName() + ".");
            return true;
        } else {
            if (mobile.getUserId() != null) {
                this.communicationService.sendTextMessage(mobile, "\n\nYour prayer fails.");
